...
import type { DescriptorData } from '../data';
...
const MechanicalDescriptions = (props) => {
  const { act, data } = useBackend<DescriptorData>();
    ...
    <Section fill title="Mechanical Descriptions">
      ...
      <LabeledList>
        {data.descriptors_custom.map((desc) => (
          <LabeledList.Item key={desc.index} label={desc.name}>
            <Stack>
      ...
      <Button onClick={() => act('print_descriptor_setup')} fluid mt={1} mb={1}>
