...
const Controls = (props) => {
  const { act, data } = useBackend<ClassData>();
...
export const Department = (props: { dept: DepartmentEnum }) => {
  const [constantData] = useConstantPrefs();
  const { dept } = props;
  const { data } = useBackend<ClassData>();
...
export const ClassTitle = (props: { cls: Class }) => {
  ...
  return (
    <Tooltip
      content={
        <Stack vertical>
          <Stack.Item fontSize={0.9} italic>
            Click to see subclass details!
          </Stack.Item>
          <Stack.Item>
            <Stack align="">
...
const PriorityButton = (props: PriorityButtonProps) => {
  const className = `PreferencesMenu__Job__PriorityButton`;
