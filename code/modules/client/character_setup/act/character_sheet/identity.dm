/datum/preferences/proc/ui_act_character_sheet_identity(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("origin")
			...
			var/result = tgui_input_list(user, "From where do you come?", "ORIGINS", virtue_choices, virtue_origin.name)
			if(result)
		...
		if("extra_language")
			...
			for(var/datum/language/language as anything in selectable_languages)
		...
		if("statpack")
			...
			var/statpack_input = tgui_input_list(user, "How shall your strengths manifest?", "STATPACK", statpacks_available, statpack.name, descriptions = statpack_descriptions)
			if(statpack_input)
		...
		if("select_virtue")
			...
			// TODO: make this work for downstream with no modifications in this proc (use subprocs)
			var/key = params["key"]
			...
			if(result)
				...
				if(skin_tone == SKIN_COLOR_ROT)
					...
					for(var/datum/virtue/combat/second_chance/R as anything in virtues)
						should_rot = TRUE
						break
					if(!should_rot)
		...
		if("barksound")
			...
			for(var/path in GLOB.bark_list)
				...
				if(B::ignore)
					continue
				woof_woof[B::name] = B::id

			var/datum/bark/B = GLOB.bark_list[bark_id]
			var/new_bork = tgui_input_list(user, "Choose your desired vocal bark", "Character Preference", woof_woof, B::name)
			if(new_bork)
		...
		if("barkpitch")
			...
			if(!B)
				to_chat(user, span_danger("ERROR: Vocal bark set to an invalid option."))
				return CHARACTER_ACT_DATA_UPDATE
			var/borkset = tgui_input_number(user, "Choose your desired baseline bark pitch.", "Character Preference", bark_pitch, B::maxpitch, B::minpitch, round_value = FALSE)
			if(!isnull(borkset))
				bark_pitch = borkset
			return CHARACTER_ACT_DATA_UPDATE
		...
		if("barkvary")
			...
			if(!B)
				to_chat(user, span_danger("ERROR: Vocal bark set to an invalid option."))
				return CHARACTER_ACT_DATA_UPDATE
			var/borkset = tgui_input_number(user, "Choose your desired baseline bark variance.", "Character Preference", bark_variance, B::maxvariance, B::minvariance, round_value = FALSE)
			if(!isnull(borkset))
