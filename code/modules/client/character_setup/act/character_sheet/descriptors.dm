/datum/preferences/proc/ui_act_character_sheet_descriptors(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("choose_descriptor")
			...
			var/datum/mob_descriptor/current = entry.descriptor_type

			var/picked_descriptor_name = tgui_input_list(user, "Describe my [lowertext(choice.name)]", "Describe myself", picklist, current::name)
