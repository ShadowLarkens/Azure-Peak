...
/datum/preferences/proc/ui_act_character_creator_appearance_body(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("mutant_color3")
			...
			return CHARACTER_ACT_PREVIEW_UPDATE
		if("body_size")
			var/new_body_size = tgui_input_number(user, "Choose your desired sprite size:\n([BODY_SIZE_MIN*100]%-[BODY_SIZE_MAX*100]%), Warning: May make your character look distorted", "Character Preference", features["body_size"]*100, BODY_SIZE_MAX*100, BODY_SIZE_MIN*100)
			if(new_body_size)
				new_body_size = clamp(new_body_size * 0.01, BODY_SIZE_MIN, BODY_SIZE_MAX)
				verbose_pref_log_change(user, "notice", "Body Size", "[round(features["body_size"] * 100)]%", "[round(new_body_size * 100)]%")
