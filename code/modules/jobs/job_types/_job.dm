...
/// Dynamic UI data for TGUI to display these in the character creator
...
/// Constant UI data for TGUI to display these in the character creator
/datum/job/proc/constant_ui_data()
	return list(
		"titles" = list(
			TITLES_M = display_title || title,
			TITLES_F = f_title || display_title || title,
			// INSTRUCTIONS FOR DOWNSTREAM uncomment this
			// TITLES_N = n_title || display_title || title,
		),
