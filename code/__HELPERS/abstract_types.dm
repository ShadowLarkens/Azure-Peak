...
/// Like init_subtypes, but automatically excludes abstract typepaths (also returns instead of modifying in-place)
/// Gives a list of (instance, instance, instance)
/proc/init_valid_subtypes(datum/sometype)
	. = list()
	for(var/path in valid_subtypesof(sometype))
		. += new path()

/// Like init_valid_subtypes, but gives a list of
/// list(/datum/one = new /datum/one, /datum/two = new /datum/two)
/proc/init_valid_subtypes_by_path(datum/sometype)
	. = list()
	for(var/path in valid_subtypesof(sometype))
		.[path] = new path()
